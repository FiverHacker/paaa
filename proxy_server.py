...
class SpeedLimiter:
    ...
    def __init__(self, kbps: float):
        ...
        # Use a larger bucket size (2 seconds worth) for smoother limiting
        self.bucket_size = self.bps * 2.0 if self.bps != float('inf') else float('inf')
        self.tokens = self.bucket_size  # Start with full bucket
        self.last_update = time.time()
        self.lock = asyncio.Lock()
        # Log speed limit for debugging
        if self.bps != float('inf'):
            mbps = (self.bps * 8) / 1_000_000
            logger.info(f"Speed limiter initialized: {kbps:.2f} KB/s = {mbps:.2f} Mbps")
    ...
    async def wait(self, bytes_to_send: int):
        ...
        async with self.lock:
            ...
            # Add tokens based on elapsed time (but cap at bucket size)
            self.tokens = min(self.bucket_size, self.tokens + elapsed * self.bps)
            ...
            if self.tokens < bytes_to_send:
                ...
                if wait_time > 0.0001:  # 0.1ms threshold (lowered for better accuracy)
                    await asyncio.sleep(wait_time)
                    # Update tokens after waiting
                    now = time.time()
                    elapsed = now - self.last_update
                    self.tokens = min(self.bucket_size, elapsed * self.bps)
                    self.last_update = now
                else:
                    self.tokens = 0
            else:
...
class ProxyConnection:
    ...
    def __init__(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter, 
        ...
        if self.speed_limiter:
            mbps = (config.speed_limit_kbps * 1024 * 8) / 1_000_000
            logger.info(f"Proxy connection created with speed limit: {config.speed_limit_kbps:.2f} KB/s ({mbps:.2f} Mbps)")
        ...
        try:
            ...
            while True:
                ...
                if self.upstream_writer:
                    # Apply speed limiting before sending to upstream
                    if self.speed_limiter:
    ...
    async def _forward_upstream_to_client(self):
        ...
        try:
            # Use larger chunk size for better performance on high-speed connections
            chunk_size = 65536  # 64KB chunks for high-speed connections
            while True:
                data = await self.upstream_reader.read(chunk_size)
                if not data:
                    break
                ...
                # Apply speed limiting before sending to client
                if self.speed_limiter:
                    await self.speed_limiter.wait(len(data))
